      ...
      <div className="bg-white rounded shadow p-6 space-y-6">
        ...
        <div className="flex items-center gap-4">
          <button onClick={handleSubmit} disabled={saving} className="btn-primary bg-red-700 text-white py-2 px-4 rounded">
            {saving ? "Saving..." : "Submit & Generate PDF"}
          </button>
          <div className="text-sm text-gray-500">You can download the generated PDF or view files in the verification dashboard.</div>
        </div>

        {/* Verification Dashboard placeholder */}
      ...
      {modal.open && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
          <div className="bg-white rounded p-6 w-96">
            ...
            <p className="text-sm mb-4">{modal.message}</p>
            <div className="flex gap-2">
              {modal.pdfUrl && (
                <a href={modal.pdfUrl} target="_blank" rel="noreferrer" className="text-sm px-3 py-2 bg-gray-100 rounded">Open PDF</a>
              )}
              <button onClick={() => setModal({ open: false, message: "", pdfUrl: null })} className="px-3 py-2 bg-red-700 text-white rounded">Close</button>
            </div>
          </div>
        </div>
      )}
